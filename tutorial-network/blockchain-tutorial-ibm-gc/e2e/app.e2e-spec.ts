...
describe('Starting tests for blockchain-tutorial-ibm-gc', function() {
  ...
  it('website title should be blockchain-tutorial-ibm-gc', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('blockchain-tutorial-ibm-gc');
    })
  });
  ...
  it('navbar-brand should be blockchain-tutorial-ibm-gc',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('blockchain-tutorial-ibm-gc');
    });
  });
